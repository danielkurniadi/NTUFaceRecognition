...
class FaceHaarDetectCrop(object):
	...
	def __init__(self, prototxt, caffeModel, imsize=(300,300), confThresh=confThresh,
	...
	def __call__(self, image):
		...
		Returns:
			.. rect (dlib.rectangle): rectangle of bounding boxes
					If multi_face false, output is bounding box: rectangle(x1,y1,x2,y2)
					If multi_face true, output is array of bounding box List[rectangle(x1,y1.x2,y2)].
		"""
		if self.multi_face:
...
class FaceAlign(object):
	...
	def __init__(self, face_landmark_model, desired_left_eye=(0.23, 0.23), imsize=(300,300)):
	...
	def _align_face(self, image, rect):
		...
		"""
		face_height, face_width = self.imsize
		...
		tx = face_width * 0.5
		ty = face_width_height * self.desired_left_eye[1]

		rot_mat[0, 2] += (tx - eyes_center[0])
		rot_mat[1, 2] += (ty - eyes_center[1])

		# apply the affine transformation
		(w, h) = (face_width, face_width_height)
